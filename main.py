...
def get_matches(db: Session = Depends(get_db)):
    return db.query(Match).all()    <body><div class=container>
        <h1>🤖 AI Resume Screening System</h1>
        <h2>1. Create Job</h2>
        <input type=text id=jobTitle placeholder="Job Title">
        <textarea id=jobDesc rows=5 placeholder="Job Description"></textarea>
    ...
    <script>
        ...
        async function matchResume(){
            const res=await fetch(`/api/match/${jobId.value}/${resumeId.value}`);
            const data=await res.json();
            results.innerHTML='<div class=result><pre>'+JSON.stringify(data,null,2)+'</pre></div>';
        }
        async function viewMatches(){
            const res=await fetch('/api/matches');
            const data=await res.json();
